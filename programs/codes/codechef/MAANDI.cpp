...
{
	REP(i,str.size())	if(str[i] != '4' && str[i] != '7')	return false;
	return true;
}
...
bool isoverlucky(int num)
{
	...
	int num1 = num;
	string str1;
	while(num1)	str1.PB((num1 % 10) + '0'),num1/=10,si++;
	reverse(ALL(str1));
	for(int i = 1; i < (1 << si) ; i++)
	{
		...
		REP(j,si)
		{
			if(i & (1 << j))	str.PB(str1[j]);
		}
		reverse(ALL(str));
		if(islucky(str))	return true;
	}
	return false;
}
