...
void solve()
{
	double B[55];
	REP(i,n)	B[i] = 0;
	int num = comb(c) + comb(n* c - c);
	int den = comb(n*c);
	REP(i,n)	B[i] += (double)((double)num * A[i]);
	REP(i,n)	REP(j,n)	if(i!=j)	B[i] += c*c * exchange(A[i],A[j]);
	REP(i,n)	A[i] = B[i]/(double) den;
	return;
}
