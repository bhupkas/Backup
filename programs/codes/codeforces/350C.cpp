...
int main()
{
	...
	{
		cin >> A[i].x >> A[i].y;
		A[i].cnt = abs(A[i].x) + abs(A[i].y);
		if(abs(A[i].x) == 0 || abs(A[i].y) == 0) ans+=4;
		else ans+=6;
	}
	sort(A,A+n,fun);
	cout<<ans<<endl;
	REP(i,n)
	{
		if(A[i].x<0)	cout<<1<<" "<<abs(A[i].x)<<" L"<<endl;
		else if(A[i].x>0)	cout<<1<<" "<<abs(A[i].x)<<" R"<<endl;
		if(A[i].y<0)	cout<<1<<" "<<abs(A[i].y)<<" D"<<endl;
		else if(A[i].y>0)	cout<<1<<" "<<abs(A[i].y)<<" U"<<endl;
		cout<<2<<endl;
		if(A[i].x<0)	cout<<1<<" "<<abs(A[i].x)<<" R"<<endl;
		else if(A[i].x>0)	cout<<1<<" "<<abs(A[i].x)<<" L"<<endl;
		if(A[i].y<0)	cout<<1<<" "<<abs(A[i].y)<<" U"<<endl;
		else if(A[i].y>0)	cout<<1<<" "<<abs(A[i].y)<<" D"<<endl;
		cout<<3<<endl;
	}
	return 0;
}