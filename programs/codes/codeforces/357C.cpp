...
int main()
{
    ...
    REP(i,m)	cin >> l[i] >> r[i] >> x[i];
   	...
   	/*REP(i,n+1)	
   	{
   		REP(j,v[i].size())	cout << v[i][j].FF << " " << v[i][j].SS << endl;
   		cout << endl;
   	}*/
   	REP(i,v[0].size())	ss.insert(v[0][i]);
   	FOR(i,1,n+1)
   	{
   		//for(it = ss.begin() ; it != ss.end() ;it++)	cout << (*it).FF << " " << (*it).SS << endl;
   		//cout << endl;
   		it = ss.begin();
   		while((*it).SS == i)	it++;
   		re[i] = (*it).SS;
   		...
   		{
   			it = ss.find(v[i][j]);
   			if(it == ss.end())	ss.insert(v[i][j]);
   			else				ss.erase(v[i][j]);
   		}
   	}
   	re[x[m-1]] = 0;
   	FOR(i,1,n+1)	cout << re[i] << " " ;
   	cout << endl;
    return 0;
}