...
int main()
{
	pre();
	cin >> n;
	FOR(i,1,n+1)	cin >> capacity[i] , myset.insert(i);
	cin >> m;
	while(m--)
	{
		int q;
		...
		if(q == 1)
		{
			...
			while(1)
			{
				if(toadd <= 0)	break;
				if(idx  == -1)	break;
				if(capacity[idx] - curr[idx] > toadd)
				{
					curr[idx] += toadd;
					break;
				}
				if(capacity[idx] - curr[idx] > 0)
				{
					toadd -= (capacity[idx] - curr[idx]);
					curr[idx] = capacity[idx];
					myset.erase(idx);
				}
				idx = fun(idx);
			}
