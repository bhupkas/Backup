...
{
   if (x<10) return x>=1;
   LL res =  fun(x/10-1)*10 + 1;
   if (ok(x/10)) res += x%10 + 1;
   return res;
}
...
int main()
{
   ...
   REP(i,n)	cin >> L[i] >> R[i];
   cin >> k;
   vector < double > prob(n+5);
   double dp[1005][1005];
   REP(i,n+1)	REP(j,n+1)	dp[i][j] = 0.0;
   REP(i,n) prob[i+1] = (double)(fun(R[i]) - fun(L[i] -1)) / (double)(R[i] - L[i] + 1);    
   dp[0][0] = 1.0;
   FOR(i,1,n+1)
   {
   ...
   int req = (int)(ceil)(((double)(k) * (double)n)/100.0);
   double ans = 0.0;
   FOR(i,req,n+1)    ans += dp[n][i];
   printf("%.15lf\n",ans);
   return 0;
}