...
double evaluate(vector < int > val)
{
	double re = 0.0;
	vector < int > pr(n);
	REP(i,n)	pr[i] = (double)v[i].SS;
	REP(i,n)	
	{
		pr[i] += (double)(val[i] * 10);
		if(pr[i] > 100.0)	pr[i] = 100.0;
	}
	for(int mask = 0 ; mask < (1 << n) ; mask++)
	{
		...
		if(2*cnt > n)	re += p;
		else			re += (p*(double)A)/(double)(A+B);
	}
	ans = max(ans,re);
}

void fun(int currn, int currk, vector < int > val)
{
	if(currn == n)	evaluate(val);			
	else	for(int i = 0 ; i <= currk ; i++)	val[currn] = i , fun(currn+1,currk-i,val);
}
