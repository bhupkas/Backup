...
int main()
{
	...
	cin >> n;
	string str1,str2;
	REP(i,n)	str1.PB('0'),str2.PB('0');
	REP(i,n/2)	str1[i] = '1' , str2[i] = '1';
	VI a(n),b(n);
	vector < pair < int , pair < int, int > > > c;
	REP(i,n)	cin >> a[i] >> b[i] , c.PB(MP(a[i],MP(1,i))),c.PB(MP(b[i],MP(2,i)));
	sort(c.begin(),c.end(),cmp);
	REP(i,n)	
	{
		if(c[i].second.first == 1)	str1[c[i].second.second] = '1';
		else						str2[c[i].second.second] = '1';
	}
