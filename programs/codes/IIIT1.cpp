...
int main()
{
	...
	VI adj[25];
	REP(i,n)	REP(j,n)	if(M[i][j])	adj[i].PB(j);
	bool visited[1 << n + 5];
	memset(visited, false,sizeof(visited));
	LL V[25];
	REP(i,n)	V[i] = 0;   
	LL mask = 0;
	REP(i,adj[0].size())	V[adj[0][i]] = 1 , mask |= (1 << i);	
	visited[mask] = true;
	FOR(i,1,1<<n)
	{
		if(i == p)	break;
		...
		VI temp;
		REP(i,n)	temp.PB(V[i]);
		REP(i,n)	REP(j,adj[i].size())	if(temp[i] & 1)	V[adj[i][j]]++;	else	V[adj[i][j]]+=2;
		REP(i,n)	if(V[i] & 1)	mask |= (1 << i);
		if(visited[mask])	break;
		else			visited[mask] = true;
	}
    return 0;
}