...
{
	LL resl;
	if((ceil(a)-a)<(a-floor(a)))	resl = ceil(a);
	else		resl = floor(a);
	return resl;
}
...
void polymult()
{
	...
	fun(pointvalueC,D,N);
	R[0] = num(D[0].real / (double) N );
	for(i = 1 ; i < N ; i++)	R[i] = num(D[N - i].real / (double) N);
}
...
int main()
{
	...
	while(t--)
	{
		...
		while(N <= n + m + 2)	N *= 2;
		int i;
		for(i = 0 ; i < N ; i++)	A[i].real = A[i].complex = B[i].real = B[i].complex = 0.0;
		for(i = 0 ; i <= n ; i++)
		{
			scanf("%d",&temp);
		...
		int j;
		printf("%d\n",i);
		for(j = 0 ; j <= i ; j++)	printf("%lld ",R[j] % (LL)(poterm));
		printf("\n");
	} 
	return 0;
}
